    ...
    let named_chain =
        ...
        match &backup_restore_config.clone() {
            IndexerBackupRestoreConfig::GcsFileStore(gcs_file_store) => {
                // info!("gcs_service_account_key_path is : {}", gcs_file_store.clone().gcs_service_account_key_path);
                Arc::new(Box::new(
                GcsBackupRestoreOperator::new(
                    gcs_file_store.gcs_bucket_name.clone() + "_" + &named_chain,
                    // gcs_file_store.clone().gcs_service_account_key_path,
                ),
            ))},
        ...
        info!("SERVICE_ACCOUNT is: {}", env::var_os("SERVICE_ACCOUNT").expect("service account not none").to_string_lossy());
